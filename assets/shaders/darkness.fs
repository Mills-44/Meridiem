...
extern PRECISION vec2 darkness;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    //vec2 rotater = vec2(cos(darkness.r*0.1221), sin(darkness.r*0.3512));

    vec3 darknessVec = tex.rgb;
    if (darkness.g > 0.0 || darkness.g < 0.0) {
        darknessVec.r = (tex.r * 0.04) + (tex.g * 0.05) + (tex.b * 0.1);
        darknessVec.g = (tex.r * 0.02) + (tex.g * 0.05) + (tex.b * 0.02);
        darknessVec.b = (tex.r * 0.02) + (tex.g * 0.05) + (tex.b * 0.02);
    }

    tex = vec4(darknessVec, tex.a);
